...
import helpers.mongo.notesdb as ᴘʏᴛʜᴏɴɪᴄ
...
async def save(client: Client, message):
    ...
    if not arg:
        ...
        return
    note_name = arg
    note = await ᴘʏᴛʜᴏɴɪᴄ.get_note(note_name)
    if note:
    ...
    await ᴘʏᴛʜᴏɴɪᴄ.save_note(note_name, copy.message_id)
...
async def clear(client, message):
    ...
    if not note:
        ...
        return
    await ᴘʏᴛʜᴏɴɪᴄ.rm_note(note_name)
...
async def notes(client, message):
    msg = "**Saved Notes**\n\n"
    all_notes = await ᴘʏᴛʜᴏɴɪᴄ.all_notes()
    if not all_notes:
