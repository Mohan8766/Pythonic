...
import helpers.mongo.welcomedb as ᴘʏᴛʜᴏɴɪᴄ
...
async def welcome(client, message):
    await ᴘʏᴛʜᴏɴɪᴄ.clear_welcome(str(message.chat.id))
...
async def new_welcome(app: Client, message):
    msg_id = await ᴘʏᴛʜᴏɴɪᴄ.get_welcome(str(message.chat.id))
    caption = ""
    men = ""
...
async def setwelcome(app: Client, message):
    ...
    await ᴘʏᴛʜᴏɴɪᴄ.save_welcome(str(message.chat.id), msg_id)
